...
namespace DotPulsar.Internal
{
    ...
    {
        private readonly IAsyncQueue<MessagePackage> _queue;
        private readonly IMessageTracker _tracker;

        public MessageQueue(IAsyncQueue<MessagePackage> queue, IMessageTracker tracker)
        {
            _queue = queue;
            _tracker = tracker;
        }
        ...
        public async ValueTask<MessagePackage> Dequeue(CancellationToken cancellationToken = default)
        {
            ...
            _tracker.Track(message.MessageId);
            return message;
        }

        public void Acknowledge(MessageIdData obj) => _tracker.Acknowledge(obj);

        public void NegativeAcknowledge(MessageIdData obj) => _tracker.NegativeAcknowledge(obj);
